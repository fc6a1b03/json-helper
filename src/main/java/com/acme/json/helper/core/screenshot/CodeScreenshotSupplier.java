...
public class CodeScreenshotSupplier {
    ...
    public static BufferedImage createImage(final Editor editor) {
        ...
        if (SwingUtilities.isEventDispatchThread()) {
            return createImageOnEdt(editor);
        }
        final AtomicReference<BufferedImage> imageRef = new AtomicReference<>();
        ApplicationManager.getApplication().invokeAndWait(() -> imageRef.set(createImageOnEdt(editor)));
        return imageRef.get();
    }

    private static BufferedImage createImageOnEdt(final Editor editor) {
